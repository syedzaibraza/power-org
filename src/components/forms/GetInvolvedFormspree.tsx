...
const ENDPOINT = "https://formspree.io/f/xbdavgwz";
...
export function GetInvolvedFormspree() {
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const selectedInterests = fd.getAll("interests").map(String).filter(Boolean);
    ...
    <>
      ...
      >
        <input
          type="hidden"
          name="_subject"
          value="New Get Involved submission from powerny.org"
        />
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {INTERESTS.map((a) => (
              <label
                key={a}
                className="flex items-center gap-2 cursor-pointer bg-[#F4F6F9] rounded-[6px] px-3 py-2.5 hover:bg-[#D4AF37]/10 transition-colors"
              >
                <input
                  type="checkbox"
                  name="interests"
